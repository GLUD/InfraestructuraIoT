...
sed -i "s/password:.*/password: \"$(echo $3 | node-red-admin hash-pw | cut -d ' ' -f 2)\",/g" node-red-template/settings.js
docker build . -t iot4dummies/client
docker run --net testnet -d -p $1:$1 -it --name $2 iot4dummies/client
docker exec -it server sed -i "38i  location /$2/ {\n proxy_pass http://$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $2
